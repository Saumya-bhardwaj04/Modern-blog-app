...
function Input({ type, placeholder, value, setUserData, field, icon }) {
    ...
    return (
        <div className="relative w-full">
            <i className={"fi " + icon + " absolute top-1/2 -translate-y-1/2 mt-1 left-4 opacity-50"}></i>
            <input
                ...
                onChange={(e) => setUserData((prev) => ({ ...prev, [field]: e.target.value }))} />
            {
                ...
                (<i onClick={() => setShowPassword((prev) => !prev)}
                    className={`fi ${showPassword ? "fi-sr-eye" : "fi-sr-eye-crossed"}  absolute top-1/2 -translate-y-1/2 mt-1 right-4 opacity-50 cursor-pointer`}></i>)
            }
        </div>
    )
}
